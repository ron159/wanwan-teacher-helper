...
- 通信簿：导入 .xlsx/.xls，或粘贴学生与家长信息；识别父母及其他监护人姓名、电话和住址，先预览去重，再搜索、编辑或删除。识别失败时提供示例，原文保留在备注中。
...
界面跟随 Windows 的浅色/暗色模式切换。每次正常启动会向本仓库 GitHub Releases 查询新版本；确认后下载、校验并替换 EXE。离线时文件处理仍可使用，材料不会上传。旧版更新到 v0.3.2 时，若替换后首次重启提示找不到临时目录中的 `python314.dll`，关闭报错后手动重新打开原位置的程序即可；也可从 Release 直接下载 v0.3.2。

通信簿是独立页面，号码默认视为家长电话。数据保存在当前电脑用户目录中的 `contacts.json`，不与小程序自动同步；请按园所要求保护和备份。Excel 导入限 2 MB、每个工作表约 1000 行、通信簿最多 1000 条，识别后请核对预览和备注。
...
CI 在 Windows 执行测试，然后分别启动目录版和单 EXE，使用合成材料实际运行所有模块并生成截图、结果与源文件哈希检查。除明确授权的 `v0.2.0-preview.1`、`v0.2.0-preview.2`、`v0.3.0`、`v0.3.1` 和 `v0.3.2` 自用未签名版外，标签发布必须先通过生产签名和时间戳校验，再执行单 EXE 验收；配置见 [签名说明](packaging/signing.md)。Release 附 SHA-256、SBOM、第三方许可与验证记录。
...
未配置生产代码签名身份，v0.1.0、v0.2.0-preview.1、v0.2.0-preview.2、v0.3.0、v0.3.1 与 v0.3.2 自用 EXE 未做 Authenticode 签名；请从本仓库下载并核对 SHA-256。除上述明确授权的标签外，新标签发布会因缺少签名身份而阻断。CI 使用新建的非管理员账户、移除 Python 的 PATH，并阻断测试账户所有进程出站执行全部 30 个模式组合，同时检查通信簿识别与保存；还会测量启动、解包与缓存空间并核对退出清理。影音引擎只允许本地协议。物理断网实机与真实 Office/WPS 样本验收仍需在对应环境执行。
