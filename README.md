...
未配置生产代码签名身份，已发布的 v0.1.0 EXE 未做 Authenticode 签名；请从本仓库下载并核对 SHA-256。新标签发布会因缺少签名身份而阻断。CI 使用新建的非管理员账户、移除 Python 的 PATH，并阻断所有进程出站执行全部 28 个模式组合；同时测量启动、解包与缓存空间并核对退出清理。影音引擎只允许本地协议。物理断网实机与真实 Office/WPS 样本验收仍需在对应环境执行。
