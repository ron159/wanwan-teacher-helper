...
标签发布必须通过 `scripts/sign_release.ps1`；缺少身份、证书过期、签名不可信或无时间戳会中止发布。仅 `v0.2.0-preview.1` 和用户授权的自用版 `v0.2.0-preview.2` 允许未签名发行，`signature.json` 明确记录 `UnsignedPreview`；其他版本标签仍强制检查生产签名。PR 构建保持未签名，仅供验证。
